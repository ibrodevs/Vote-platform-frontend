...
export default function RegulationsPage() {
  ...
  useEffect(() => {
    ...
    // Fetch from API
    api.getStaticPage('regulations')
      .then(data => {
        if (data) setPageData(data);
      })
      .catch(err => {
        console.warn('Could not load regulations from API, using fallback:', err);
      })
  ...
  const title = lang === 'ky' && pageData?.title_ky ? pageData.title_ky : (pageData?.title || (lang === 'ky' ? 'Студенттердин электрондук добуш берүү регламенти' : 'Регламент электронного голосования студентов'));
    ...
    if (!text) {
      return (
        <div className="space-y-6 text-[15px] leading-relaxed text-[var(--body)]">
          <p>
            {lang === 'ru'
              ? 'Настоящий Регламент определяет порядок подготовки, организации и проведения прямого тайного электронного голосования на платформе Dobush.kg в высших учебных заведениях Кыргызской Республики.'
              : 'Бул Регламент Кыргыз Республикасынын жогорку окуу жайларында Dobush.kg платформасында түз, жашыруун электрондук добуш берүүнү даярдоо, уюштуруу жана өткөрүү тартибин аныктайт.'
            }
          </p>
        </div>
      );
      ...
      <div className="max-w-[960px] mx-auto px-4 sm:px-6 md:px-8">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <Badge variant="blue" dot={true}>
            {lang === 'ru' ? 'ОФИЦИАЛЬНЫЙ РЕГЛАМЕНТ' : 'РАСМИЙ РЕГЛАМЕНТ'}
          </Badge>
        </div>
        ...
        <div className="mb-10 text-center sm:text-left">
          ...
          <div className="flex flex-wrap items-center gap-4 text-xs text-[var(--muted)] font-medium">
            <span className="flex items-center gap-1.5">
              ...
              {lang === 'ru' ? 'Редакция 2026 года' : '2026-жылдагы редакциясы'}
            </span>
            <span className="flex items-center gap-1.5">
              <ShieldCheck className="w-3.5 h-3.5 text-[var(--green)]" />
              {lang === 'ru' ? 'Действующий нормативный документ' : 'Күчүндөгү ченемдик документ'}
            </span>
          </div>
        </div>
        ...
        <div className="crm-card p-6 sm:p-10 md:p-12 shadow-[var(--shadow-card)]">
          {isLoading ? (
            <div className="py-12 text-center text-[var(--muted)] text-sm">
              {lang === 'ru' ? 'Загрузка текста регламента...' : 'Регламенттин тексти жүктөлүүдө...'}
            </div>
          ) : (
